...
| `npx ngodingpakeai task reset <ref> "<reason>"` | Put a `doing`/`failed` task back to `todo`. **Only when the user asks.** |
...
A task you started (`doing`) or reported stuck (`failed`) can be sent back to
`todo` so it can be picked up fresh later:
...
`task fail` and explain. Only `doing` and `failed` tasks can be reset; a `done`
task is protected and the command will fail (409).
